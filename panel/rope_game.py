...
class RopeGameLink:
    ...
    def post(self, path: str, body: dict, state):
        if path == "/api/rope/control":
            if not isinstance(body, dict) or body.get("action") not in ("restart", "cut"):
                return {"error": "expected restart or cut action"}, 400
            if state.view != "rope":
        ...
        if path == "/api/rope/status":
            ...
            with self.lock:
                self.status = {"outcome": body["outcome"], "stars": stars, "cuts": cuts,
                               "tracked": body.get("tracked") is True, "held": body.get("held") is True}
