...
def main() -> int:
    ...
    s, d = req("/api/app/ask", {"action": "start"})
    check("ask start relays", s == 200 and d.get("ok") is True, (s, d))
    s, d = req("/api/app/ask", {"action": "dance"})
    check("bad ask action rejected", s == 400, s)
    s, d = req("/api/ask/state", {"phase": "listening", "seconds_left": 12})
    check("ask phase accepted", s == 200 and d.get("ok") is True, (s, d))
    s, d = req("/api/state")
    check("ask phase in state", d.get("ask", {}).get("phase") == "listening", d.get("ask"))
    s, d = req("/api/ask/state", {"phase": "napping"})
    check("bad ask phase rejected", s == 400, s)
    s, d = req("/api/view", {"view": "ask"})
    check("ask view selectable", s == 200 and d.get("view") == "ask", (s, d.get("view")))
    req("/api/view", {"view": "home"})
